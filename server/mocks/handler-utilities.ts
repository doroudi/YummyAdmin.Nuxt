...
export function CreatePagedResponse<T extends { [key: string]: any }>(
    ...
    const skipCount = url.searchParams.get('page') ?? '1'
...
export function CreateListResponse<T extends { [key: string]: any }>(
    ...
    const url = new URL(`https://localhost:3000${req.path}`) // const url = new URL(getRequestUrl(request))
    const query = url.searchParams.get('query')
...
export async function delay(delayAmountMs: number = 1500) {
    await new Promise(resolve => setTimeout(resolve, Number(delayAmountMs)))
}