...
async function doLogin() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      const loginSucceed = await login(loginInfo.value)
      if (loginSucceed) {
        useNotification().success({ title: $t('login.successMessage') })
...
async function socialLoginWithProvider(provider: string) {
  ...
  if (result) {
    ...
    useNotification().error({ title: $t('login.failedMessage') })
    setLoginFailed()
  }
    ...
    <div class="login-box w-full px-3 md:px-0">
      <div class="md:shadow-lg bg-white dark:bg-slate-800  rounded-md w-full" :class="{ failed: loginFailed }">
        <div class="p-5">
          ...
          <n-form ref="formRef" :model="loginInfo" @submit="doLogin">
            ...
            <n-button attr-type="submit" size="large" :block="true" type="primary" :loading="false">
              {{ $t('login.loginButton') }}
            </n-button>
          </n-form>
          ...
          <div class="social-login pt-3">
            ...
            <div class="flex items-center justify-center">
              <n-button quaternary circle mx-2 @click="socialLoginWithProvider('google')">
                <template #icon>
                  <GoogleIcon size="1.4rem" />
                </template>
              </n-button>

              <n-button quaternary circle mx-2 @click="socialLoginWithProvider('microsoft')">
                <template #icon>
                  <MicrosoftIcon size="1.4rem" />
                </template>
              </n-button>

              <n-button quaternary circle mx-2 @click="socialLoginWithProvider('apple')">
                <template #icon>
                  <AppleIcon size="1.4rem" />
                </template>
              </n-button>
            </div>
          </div>
