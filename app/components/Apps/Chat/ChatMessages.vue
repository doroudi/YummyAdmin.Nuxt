...
const { messages, currentChat, sendMessage } = useChatApp()
const message = ref('')
  ...
  <div v-if="currentChat" class="messages-box flex flex-col items-stretch justify-stretch">
      ...
      <n-tooltip placement="top" trigger="hover">
        <template #trigger>
          <n-button class="hidden md:flex" quaternary circle>
            <template #icon>
              <Icon name="fluent:call-28-regular" size="1.4rem" />
            </template>
          </n-button>
        </template>
    ...
    <section class="flex flex-col flex-1">
      ...
      <section id="send-message" class="send-message p-4 bg-gray-100 dark:bg-gray-700 flex items-center">
        ...
        <NButton :disabled="!sendIsActive" text type="primary" @click="handleSendMessage">
          <template #icon>
            <Icon name="fluent:send-28-filled"  class="rtl-rotate" size="1.4rem" />
          </template>
        </NButton>
      </section>
    </section>
