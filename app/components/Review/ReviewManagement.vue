...
const { getReviews, isLoading, reviews } = useReviews()
  ...
  <n-layout>
    <n-layout-content>
      <div class="px-3">
          ...
          <n-tooltip v-if="checkedRows.length" trigger="hover">
            <template #trigger>
              <NButton mx-2 quaternary circle>
                <template #icon>
                  <Icon name="fluent:delete-20-regular" />
                </template>
              </NButton>
            </template>
        ...
        <n-data-table v-else remote :columns="columns" :data="reviews" :pagination="options"
